...
glav = Blueprint('glav', __name__)

@glav.route("/", methods=["GET", "POST"])
@glav.route("/glav", methods=["GET", "POST"])
def mainpage():
    try:
        username = users.query.filter_by(id=current_user.id).first().username
    except:
        username = "Аноним"
    current_page_number = request.args.get("page", 1)

    if request.args.get("page") is not None:
        current_page_number = int(request.args.get("page"))
    else:
    ...
    # Проверьте, есть ли еще книги для загрузки
    has_more_books = page_number * page_size < len(books)

    # Ограничение количество книг на страницу
    books = books[(current_page_number - 1) * page_size : current_page_number * page_size]

    # Если текущий номер страницы больше, чем
    # количество страниц, то книг больше нет
    if current_page_number > len(books) // page_size:
        has_more_books = False
    return render_template('glavnai.html', books=books, username=username, has_more_books=has_more_books, current_page_number=current_page_number)

@glav.route("/glav/register", methods=["GET", "POST"])
def register():
    errors = []
...
@glav.route("/glav/login", methods=["GET", "POST"])
def login():
    errors = []
    if request.method == "GET":
...
@glav.route("/glav/logout")
@login_required
def logout():
    logout_user()
    return redirect("/glav/login")

@glav.route("/add_book", methods=["GET", "POST"])
def add_book():
    ...
    full_book_url_form = request.form.get("full_book_url")
    ...
    if title_form == '':
        ...
        if not errors:
            new_book = Book(title=title_form, author=author_form, pages=int(pages_form), publisher=publisher_form, cover_url=cover_url, full_book_url=full_book_url_form)
...
@glav.route("/edit_book/<int:id>", methods =["GET", "POST"])
def edit_book(id):
    if current_user.is_authenticated or current_user.is_superuser:
        ...
        return render_template("glavnai.html",
                                username=current_user.username,
                                books=Book.query.all())


@glav.route("/delete_book/<int:id>", methods=["GET", "POST"])
def delete_book(id):
    if current_user.is_authenticated or current_user.is_superuser:
        book = Book.query.get(id)
        ...
        return render_template("glavnai.html", username=current_user.username, books=Book.query.all())
